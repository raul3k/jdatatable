/**
 * This is a 3-party lib, will be replaced to other more performatic lib
 */
...
class Rows {
    ...
    build(row) {
        ...
        headings.forEach((h, i) => {
            const td = createElement("td"); // Fixes #29
    ...
    add(data) {
        if (Array.isArray(data)) {
            const dt = this.dt; // Check for multiple rows
    ...
    removeTr(trId) {
        const dt = this.dt;
        dt.data = dt.data.filter(tr => tr.id !== trId);
    ...
    findRowIndex(columnIndex, value) {
        ...
        return this.dt.data.findIndex(tr => tr.children[columnIndex].innerText.toLowerCase().includes(String(value).toLowerCase()));
    }
    /**
    ...
    findRow(columnIndex, value) {
        // get the row index
        const index = this.findRowIndex(columnIndex, value); // exit if not found
        ...
        const row = this.dt.data[index]; // return innerHTML of each td

        const cols = [...row.cells].map(r => r.innerHTML); // return everything
...
class Columns {
    ...
    swap(columns) {
        if (columns.length && columns.length === 2) {
            const cols = []; // Get the current column indexes
    ...
    order(columns) {
        ...
        let cell;
        const temp = [[], [], [], []];
        const dt = this.dt; // Order the headings
    ...
    filter(column, dir, init, terms) {
        const dt = this.dt; // Creates a internal state that manages filters if there are none
        ...
        const filteredRows = Array.from(dt.filterState.originalData).filter(tr => {
            ...
            const content = cell.hasAttribute("data-content") ? cell.getAttribute("data-content") : cell.innerText; // If the filter is a function, call it, if it is a string, compare it
    ...
    sort(column, dir, init) {
        const dt = this.dt; // Check column is present
        ...
        const waitFor = []; // Check for date format
        ...
        if (th.getAttribute("data-type") === "date") {
            ...
            waitFor.push(import('./datatable/date.js').then(({
                                                                parseDate
...
class DataTable {
    constructor(table, options = {}) {
        const dom = typeof table === "string" ? document.querySelector(table) : table; // user options
    ...
    render(type) {
        ...
        let template = ""; // Convert data to HTML
        ...
        if (options.scrollY.length) {
            template += `<div class='dataTable-container' style='max-height: ${options.scrollY}; overflow-Y: auto;'></div>`;
        } else {
        ...
        if (options.searchable) {
            const form = `<div class='dataTable-search'>
                <input
    ...
    renderPager() {
        ...
        if (this.totalPages > 1) {
            ...
            const next = this.onLastPage ? this.totalPages : this.currentPage + 1; // first button
            ...
            let pager = this.links; // truncate the links
    ...
    bindEvents() {
        const options = this.options; // Per page selector
        ...
        const inputSearchBind = function(that){
            const inputSearch = document.querySelector('#jdatatable-input-search');

            if (inputSearch) {
                inputSearch.addEventListener("change", () => that.search(inputSearch.value), false);
                inputSearch.addEventListener("keyup", () => that.search(inputSearch.value), false);
            }
        };
    ...
    search(query) {
        ...
        this.data.forEach((row, idx) => {
            const inArray = this.searchData.includes(row); // https://github.com/Mobius1/Vanilla-DataTables/issues/12
        ...
        if (!this.searchData.length) {
            this.wrapper.classList.remove("search-results");
            const noResultFragment = document.createDocumentFragment();
            ...
            <div><a href="/resources/distribution-rules/new?viaResource=applications&amp;viaResourceId=${this.options.resourceId}&amp;viaRelationship=distribution_rules" class="btn btn-sm btn-outline inline-flex items-center focus:outline-none focus:shadow-outline active:outline-none active:shadow-outline" dusk="create-button">
    Create Exclusion
  </a></div>
        </div>`;
    ...
    import(userOptions) {
        ...
        if (options.data.length || isObject(options.data)) {
            ...
            if (options.type === "csv") {
                ...
                const json = isJson(options.data); // Valid JSON string
    ...
    print() {
        ...
        const w = window.open(); // Append the table to the body
    ...
    emit(event) {
        ...
        function btnClickDelete() {
            setTimeout(function() {
                document.querySelectorAll('.btn-delete').forEach((el) => {
                    el.addEventListener('click', () => this.Nova.$emit('btn-delete-clicked', el.id), false);
                });
            }, 800);
        }
        ...
        Nova.$on('btn-click-delete', () => btnClickDelete());

        Nova.$on('tr-deleted', (id) => {
            this.update();
